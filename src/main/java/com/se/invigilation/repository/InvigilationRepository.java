...
public interface InvigilationRepository extends ReactiveCrudRepository<Invigilation, String> {
    ...
    @Query("""
            ...
            """)
    Flux<Invigilation> findDispatchedAndAssigns(String depId, String collid, Pageable pageable);
    ...
    @Query("""
            select * from invigilation i
            where i.department ->> '$.depId'=:depId and i.status=1
            order by i.date, i.time ->> '$.starttime', i.course ->> '$.courseName', i.course ->> '$.teacherName'
            """)
    Flux<Invigilation> findDispatcheds(String depId);
    ...
    @Query("""
            select * from invigilation i
            where i.department ->> '$.depId'=:depId
            """)
    Flux<Invigilation> findByDepId(String depId);
}
